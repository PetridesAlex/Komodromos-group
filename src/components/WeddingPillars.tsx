...
export default function WeddingPillars() {
  ...
  return (
    <>
      ...
      <section
        className="wedding-editorial-pillars"
        aria-label={t(weddingEditorialPillarsCopy.sectionAria)}
      >
        <div className="wedding-editorial-pillars__ambient" aria-hidden />
        <div className="container">
          <header className="wedding-editorial-pillars__head reveal">
            <span className="wedding-editorial-pillars__eyebrow-rule" aria-hidden />
            <p className="wedding-editorial-pillars__eyebrow">
              {t(weddingEditorialPillarsCopy.eyebrow)}
            </p>
            <span className="wedding-editorial-pillars__eyebrow-rule" aria-hidden />
          </header>

          <div className="wedding-editorial-pillars__inner">
            {weddingEditorialPillarsCopy.items.map((pillar, index) => {
              const isHash = pillar.href.startsWith('#')
              const href = isHash ? pillar.href : weddingBrandHref(pillar.href)
              const indexLabel = String(index + 1).padStart(2, '0')
              const className = [
                'wedding-editorial-pillars__item',
                `wedding-editorial-pillars__item--${pillar.id}`,
                'reveal',
                `reveal-delay-${Math.min(index + 1, 3)}`,
              ].join(' ')
              const style = { ['--edit-i' as string]: String(index) }

              const content = (
                <>
                  <span className="wedding-editorial-pillars__accent" aria-hidden />
                  <span className="wedding-editorial-pillars__glow" aria-hidden />
                  <span className="wedding-editorial-pillars__sheen" aria-hidden />
                  <span className="wedding-editorial-pillars__index" aria-hidden>
                    {indexLabel}
                  </span>
                  <span className="wedding-editorial-pillars__body">
                    <span className="wedding-editorial-pillars__label">
                      {t(pillar.title)}
                    </span>
                    <span className="wedding-editorial-pillars__tagline">
                      {t(pillar.tagline)}
                    </span>
                    <span className="wedding-editorial-pillars__rule" aria-hidden />
                  </span>
                  <span className="wedding-editorial-pillars__action" aria-hidden>
                    <span className="wedding-editorial-pillars__action-ring" />
                    <span className="wedding-editorial-pillars__action-icon">+</span>
                  </span>
                </>
              )
